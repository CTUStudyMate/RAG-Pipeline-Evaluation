...
INPUT_DATAJSON = "testmodel2.json" 
INPUT_DATASET = "final_exp_for_report/answers/phase1__chunk_strategies/notebooklm/hsf_normal_notebooklm.csv"
OUTPUT_FILE = "hsf_minimal.csv"
DATASET_NAME = "hsfminimalsplit"
...
def load_rag_csv_as_dataset(csv_path="rag_output.csv", allowed_questions=None):
    """
    Load only rows whose question fuzzy-matches one in allowed_questions.
    Return the Dataset and print the number of skipped rows.
    """
    ...
    question_candidates = list(allowed_questions) if allowed_questions is not None else None
    rag_rows = []
    skipped_rows = 0
    with open(csv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if question_candidates is not None:
                matches = get_close_matches(question, question_candidates, n=1, cutoff=0.87)
                if not matches:
                    skipped_rows += 1
                    continue
            rag_rows.append(row)
    ...
    print(f"Skipped {skipped_rows} CSV rows without a fuzzy question match in {INPUT_DATAJSON}.")
    return dataset
...
async def run_experiment(row):
    async with semaphore:  
        ...
        retrieved_contexts = parse_retrieved_contexts(
                        retrieved_context
        )
        ...
        print(question[:120])
        # Fuzzy lookup
        matched = fuzzy_lookup(question, source_text_dict, cutoff=0.9)
...
async def main():
    ...
    rag_dataset = load_rag_csv_as_dataset(
        INPUT_DATASET,
        allowed_questions=source_text_dict,
    )
    ...
    # Save results to CSV
    output_dir = Path(f"test_effort/minimal_split")
